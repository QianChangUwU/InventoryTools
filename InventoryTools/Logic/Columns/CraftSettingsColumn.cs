...
public class CraftSettingsColumn : IColumn
{
        ...
        int rowIndex, int columnIndex)
    {
        ...
        using (var popup = ImRaii.Popup("ConfigureRecipeSettings" + rowIndex))
        {
            if (popup.Success)
            {
                ImGui.Text("Configure Recipe:".Tr());
                ImGui.Separator();
        ...
        using (var popup = ImRaii.Popup("ConfigureHQSettings" + rowIndex))
        {
            if (popup.Success)
            {
                ImGui.Text("Configure HQ Required:".Tr());
                ImGui.Separator();
        ...
        using (var popup = ImRaii.Popup("ConfigureRetainerSettings" + rowIndex))
        {
            if (popup.Success)
            {
                ImGui.Text("Retrieve from Retainer:".Tr());
                ImGui.Separator();
        ...
        using (var popup = ImRaii.Popup("ConfigureMarketWorldPreference" + rowIndex))
        {
            if (popup.Success)
            {
                ImGui.Text("Prefer Market World:".Tr());
                ImGui.Separator();
            }
        }
        ...
        if (ImGui.IsItemHovered())
        {
            ...
            {
                ImGui.TextUnformatted("Sourcing: ".Tr() + (ingredientPreferenceDefault != null ? _ingredientPreferenceLocalizer.FormattedName(ingredientPreferenceDefault) : "Use Default"));
                ImGui.TextUnformatted("Retainer: ".Tr() + (perItemRetainerRetrieval?.FormattedName() ?? "Use Default"));
                ImGui.TextUnformatted("Zone: ".Tr() + (zonePreference != null ? _mapSheet.GetRowOrDefault(zonePreference.Value)?.FormattedName ?? "Use Default" : "Use Default"));
                if (searchResult.Item.CanBePlacedOnMarket)
                {
                    ImGui.TextUnformatted("Market World Preference: ".Tr() + (worldPreference != null ? _worldSheet.GetRowOrDefault(worldPreference.Value)?.Name.ExtractText() ?? "Use Default" : "Use Default"));
                    ImGui.TextUnformatted("Market Price Override: ".Tr() + (priceOverride != null ? priceOverride.Value.ToString("N0") : "Use Default"));
                }
            }
        }
        return null;
    ...
    public void DrawRetainerIcon(FilterConfiguration configuration, int rowIndex, CraftItem item, CraftRetainerRetrieval? perItemRetainerRetrieval, CraftRetainerRetrieval retainerRetrievalDefault)
    {
        ...
        if (retainerRetrievalDefault != perItemRetainerRetrieval)
        {
            ...
            if (ImGui.IsItemHovered(ImGuiHoveredFlags.None))
            {
                using (ImRaii.Tooltip())
                {
                    ImGui.Text("Retainer Retrieval: ".Tr());
                    ImGui.Separator();
    ...
    public void DrawHqIcon(FilterConfiguration configuration, int rowIndex, CraftItem item)
    {
        ...
        if (ImGui.IsItemHovered(ImGuiHoveredFlags.None))
        {
            using (ImRaii.Tooltip())
            {
                ImGui.Text("Item Quality: ".Tr());
                ImGui.Separator();
                if (isCollectable)
                {
                    ImGui.Text("Collectable".Tr());
                }
                else if (hqRequired == true)
                {
                    ImGui.Text("HQ Only (Overridden)".Tr());
                }
                else if (hqRequired == false)
                {
                    ImGui.Text("NQ Only (Overridden)".Tr());
                }
                else if(canBeHq)
                {
                ...
                {
                    ImGui.Text("NQ Only (List Default)".Tr());
                }
    ...
    public void DrawRecipeIcon(FilterConfiguration configuration, int rowIndex, CraftItem item)
    {
        ...
        if (itemRecipe != null)
        {
            if (item.IngredientPreference.Type == IngredientPreferenceType.Crafting && ImGui.IsItemHovered(ImGuiHoveredFlags.None))
            {
                ...
                {
                    ImGui.NewLine();
                    ImGui.Text("Left Click: Next Recipe".Tr());
                    ImGui.Text("Right Click: Select Recipe".Tr());
                }
            }
        }
    ...
    public bool DrawSourceSelector(FilterConfiguration configuration, CraftItem item, int rowIndex)
    {
        ...
        if (ingredientPreferences.Count != 0)
        {
            ...
            ImGui.Text("Source Preference:".Tr());
            ImGui.SameLine();
            ...
            using (var combo = ImRaii.Combo("##SetIngredients" + rowIndex, previewValue))
            {
                if (combo.Success)
                {
                    if (ImGui.Selectable("Use Default".Tr()))
                    {
    ...
    public bool DrawMarketPriceSelector(FilterConfiguration configuration, CraftItem item, int rowIndex)
    {
        if (item.IngredientPreference.Type is IngredientPreferenceType.Marketboard)
        {
            ...
            ImGui.Text("Market Price Override:".Tr());
            ImGui.SameLine();
    ...
    public bool DrawMarketWorldSelector(FilterConfiguration configuration, CraftItem item, int rowIndex)
    {
        if (item.IngredientPreference.Type is IngredientPreferenceType.Marketboard)
        {
            ...
            ImGui.Text("Market World Preference:".Tr());
            ImGui.SameLine();
    ...
    public bool DrawRetainerRetrievalSelector(FilterConfiguration configuration, CraftItem item, int rowIndex)
    {
        ...
        using (var combo = ImRaii.Combo("##SetRetrieveRetainer" + rowIndex, previewValue))
        {
            if (combo.Success)
            {
                ...
                if (!item.Item.IsCollectable && item.Item.Base.CanBeHq && ImGui.Selectable("HQ Only".Tr()))
                {
                ...
                if (!item.Item.IsCollectable && ImGui.Selectable("NQ Only".Tr()))
                {
                ...
                if (item.Item.IsCollectable && ImGui.Selectable("Collectable Only".Tr()))
                {
