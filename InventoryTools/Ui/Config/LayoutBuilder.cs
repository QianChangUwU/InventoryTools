...
public abstract class LayoutBuilder : ILayout
{
    ...
    {
        return new LinkBlock(label.Tr(), url);
    }

    protected ButtonBlock OpenWindow<TWindow>(string label) where TWindow : Window
    {
        return new ButtonBlock(label.Tr(), () => new OpenGenericWindowMessage(typeof(TWindow)));
    }
    ...
    {
        return new SettingBlock(typeof(TSetting), nameOverride.Tr());
    }
