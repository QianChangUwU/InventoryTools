...
public class ConfigurationWizard : GenericWindow
{
    ...
    {
        WindowName = "Configuration Wizard".Tr();
        Key = "wizard";
    ...
    public override string GenericName => "Configuration Wizard".Tr();
    ...
    private void DrawPageHeader(string title)
    {
        ...
        {
            var label = $"{"step".Tr()} {_currentFeature} {"of".Tr()} {StepCount}";
            ImGui.SetCursorPosX(ImGui.GetContentRegionMax().X - ImGui.CalcTextSize(label).X);
            ImGui.TextUnformatted(label.Tr());
        }
    ...
    private void DrawSideBarItem(string label, int step)
    {
        ...
        {
            if (ImGui.Selectable(label.Tr(), isCurrent)) GoToStep(step);
        }
    }
    ...
    private void DrawWelcome()
    {
        ...
        {
            ImGui.TextWrapped("Welcome back to the Allagan Tools configuration wizard.".Tr());
            ImGui.Separator();
            ImGui.TextWrapped(
                "There are new features available to configure and you elected to show this window when that occurs.".Tr());
            ImGui.NewLine();

            if (!_showIntro && _introPages.Count != 0)
            {
                ImGui.TextWrapped("You can also go back through the introduction covering what the plugin does.".Tr());
                if (ImGui.Button("Show me around again".Tr()))
                {
                    _showIntro = true;
                    NextStep();
                }
        ...
        ImGui.TextWrapped("Welcome to the Allagan Tools configuration wizard.".Tr());
        ImGui.Separator();
        ImGui.TextWrapped(
            "This will guide you through what the plugin does and then help you set up the most commonly used features. It takes a couple of minutes, and everything here can be changed later in the settings window.".Tr());
        ImGui.NewLine();
        ImGui.TextWrapped("If you are a returning user, feel free to close this window.".Tr());
        ImGui.NewLine();
        if (ImGui.Button("Open Help".Tr())) MediatorService.Publish(new ToggleGenericWindowMessage(typeof(HelpWindow)));
    }
    ...
    private void DrawFooter()
    {
        ...
        if (isWelcome || isLast)
        {
            ...
            if (ImGui.Checkbox("Show this wizard when new features are released".Tr(), ref showOnNewFeatures))
            {
        ...
        if (isWelcome)
        {
            buttons.Add(("Close".Tr(), Close, true));
            if (StepCount == 0)
            {
                buttons.Add(("Finish".Tr(), Finish, true));
            }
            else
            {
                buttons.Add(("Continue".Tr(), NextStep, true));
            }
        }
        else
        {
            buttons.Add(("Previous".Tr(), PreviousStep, CanGoPrevious));
            if (CanGoNext)
            {
                buttons.Add(("Next".Tr(), NextStep, true));
            }
            else
            {
                buttons.Add(("Finish".Tr(), Finish, true));
