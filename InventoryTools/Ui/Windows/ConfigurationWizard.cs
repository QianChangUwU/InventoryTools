                    ...
                    if (mainContainer)
                    {
                        if (_currentFeature == 0)
                        {
                            ...
                            else
                            {
                                ...
                                ImGui.Separator();
                                ImGui.TextWrapped(
                                    "This will guide you through the setup of the most commonly used features. This wizard, with your permission will show itself again when a new feature gets released as features are normally left for the user to configure and activate.".Tr());
                                ImGui.NewLine();
                                ImGui.TextWrapped("If this is your first time using Allagan Tools, I'd recommend opening the help window and reading the General section. It gives you a run down of what the plugin can do.".Tr());
                                ImGui.TextWrapped("If you are a returning user feel free to close this window.".Tr());
                                if (ImGui.Button("Open Help".Tr()))
                                {
                ...
                using (var nextPrevBar = ImRaii.Child("nextPrevBar", new Vector2(-1, -1) * ImGui.GetIO().FontGlobalScale, true))
                {
                    if (nextPrevBar)
                    {
                        if (_currentFeature == 0)
                        {
                            if (_configurationWizardService.ConfiguredOnce)
                            {
                                if (ImGui.Button("Continue".Tr()))
                                {
                                    NextStep();
                                    _configuration.ShowWizardNewFeatures = true;
                                }

                                if (ImGui.Button("Close (and show next time the plugin loads)".Tr()))
                                {
                                    Close();
                                    _configuration.ShowWizardNewFeatures = true;
                                }
                            ...
                            {
                                if (ImGui.Button("Continue (and show on new features)".Tr()))
                                {
                                    NextStep();
                                    _configuration.ShowWizardNewFeatures = true;
                                }
                                ...
                                ImGui.SameLine();
                                if (ImGui.Button("Continue (and never show the wizard again)".Tr()))
                                {
                                    NextStep();
                                ...
                                ImGui.SameLine();
                                if (ImGui.Button("Close (and never show the wizard again)".Tr()))
                                {
                                    Close();
                                    _configuration.ShowWizardNewFeatures = false;
                                }
                        ...
                        else
                        {
                            ...
                            if (canGoNext && ImGui.Button("Next".Tr()))
                            {
                                NextStep();
                            }

                            if (!canGoNext && ImGui.Button("Finish".Tr()))
                            {
                                Finish();
                            }
