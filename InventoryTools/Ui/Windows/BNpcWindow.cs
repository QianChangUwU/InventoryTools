...
namespace InventoryTools.Ui
{
    class BNpcWindow : UintWindow
    {
        ...
        public override string GenericName => "Mob".Tr();
        public override bool DestroyOnClose => true;
        public override void DrawWindow()
        {
            if (bNpc == null)
            {
                ImGui.TextUnformatted("bNpc with the ID ".Tr() + _bNpcId + " could not be found.");
            }
            else
            {
                ImGui.Text("Type: ".Tr() + string.Join(", ", bNpc.MobTypes.Select(c => c.ToString())));

                if (bNpc.NotoriousMonster != null)
                {
                    ImGui.Text("Rank: ".Tr() + bNpc.NotoriousMonster?.RankFormatted());
                }
                ...
                if (_mobDrops != null && ImGui.CollapsingHeader("Drops (".Tr() + _mobDrops.Count + ")", ImGuiTreeNodeFlags.DefaultOpen | ImGuiTreeNodeFlags.CollapsingHeader))
                {
                ...
                if (_mobSpawns != null && ImGui.CollapsingHeader("Locations (".Tr() + _mobSpawns.Count + ")", ImGuiTreeNodeFlags.DefaultOpen | ImGuiTreeNodeFlags.CollapsingHeader))
                {
                    ImGuiStylePtr style = ImGui.GetStyle();
                    float windowVisibleX2 = ImGui.GetWindowPos().X + ImGui.GetWindowContentRegionMax().X;
                ...
                {
                    if (ImGui.CollapsingHeader("Shared Models".Tr(), ImGuiTreeNodeFlags.DefaultOpen))
                    {
                ...
                {
                    ImGui.TextUnformatted("bNpc ID: ".Tr() + _bNpcId);
        ...
        public void DrawSharedModels(EquipSlot slot)
        {
            ...
            if (ImGui.CollapsingHeader("Shared Models - ".Tr() + slot.Humanize() + " (" + relatedItems.Count + ")", ImGuiTreeNodeFlags.DefaultOpen))
            {
                ImGuiStylePtr style = ImGui.GetStyle();
                float windowVisibleX2 = ImGui.GetWindowPos().X + ImGui.GetWindowContentRegionMax().X;
