...
namespace InventoryTools.Ui
{
    ...
    public class ItemWindow : UintWindow
    {
        ...
        public override void DrawWindow()
        {
            if (Item == null)
            {
                ImGui.TextUnformatted("Item with the ID ".Tr() + _itemId + " could not be found.");
            }
            else
            {
                ImGui.TextUnformatted("Item Level ".Tr() + Item.Base.LevelItem.RowId.ToString());
                ImGui.TextUnformatted("Patch ".Tr() + Item.Patch);
                if (Item.CanBeDesynthed && Item.Base.ClassJobRepair.RowId != 0)
                {
                    ImGui.TextUnformatted("Desynth with ".Tr() + (Item.Base.ClassJobRepair.ValueNullable?.Name.ToString().ToTitleCase() ?? "Unknown"));
                }
                ...
                if (Item.CanBeAcquired)
                {
                    ...
                    ImGui.TextUnformatted("Acquired:".Tr() + (hasAcquired == null ? "Checking" : hasAcquired == true ? "Yes" : "No"));
                }

                if (Item.SellToVendorPrice != 0)
                {
                    ImGui.TextUnformatted("Sell to Vendor: ".Tr() + Item.SellToVendorPrice + SeIconChar.Gil.ToIconString());
                }

                if (Item.BuyFromVendorPrice != 0 && Item.HasSourcesByType(ItemInfoType.GilShop))
                {
                    ImGui.TextUnformatted("Buy from Vendor: ".Tr() + Item.BuyFromVendorPrice + SeIconChar.Gil.ToIconString());
                }

                if (Item.BuyFromVendorPrice != 0 && Item.HasSourcesByType(ItemInfoType.CalamitySalvagerShop))
                {
                    ImGui.TextUnformatted("Buy from Calamity Salvager: ".Tr() + Item.BuyFromVendorPrice + SeIconChar.Gil.ToIconString());
                }
                ...
                ImGuiUtil.HoverTooltip("Open in Universalis".Tr());
                ImGui.SameLine();
                ...
                ImGuiUtil.HoverTooltip("Open in Gamer Escape".Tr());
                ImGui.SameLine();
                ...
                ImGuiUtil.HoverTooltip("Open in Console Games Wiki".Tr());
                ...
                if (Item.CanOpenCraftingLog)
                {
                            ...
                            new Vector2(32, 32) * ImGui.GetIO().FontGlobalScale))
                    {
                        ...
                        if (!result)
                        {
                            _chatUtilities.PrintError("Could not open the crafting log, you are currently crafting.".Tr());
                        }
                    }

                    ImGuiUtil.HoverTooltip("Craftable - Open in Craft Log".Tr());
                }
                if (Item.CanBeCrafted)
                {
                    ...
                    using (var popup = ImRaii.Popup("AddCraftList" + _itemId))
                    {
                        if (popup)
                        {
                            ...
                            foreach (var filter in craftFilters)
                            {
                                ...
                                {
                                    if (ImGui.Selectable("Add item to craft list - ".Tr() + filter.Name))
                                    {
                    ...
                    ImGuiUtil.HoverTooltip("Craftable - Add to Craft List".Tr());
                }
                if (Item.CanBeGathered)
                {
                    ...
                    ImGuiUtil.HoverTooltip("Gatherable - Open in Gathering Log".Tr());
                    ...
                    ImGuiUtil.HoverTooltip("Gatherable - Gather with Gatherbuddy".Tr());
                }
                ...
                if (Item.ObtainedFishing)
                {
                    ...
                    ImGuiUtil.HoverTooltip("Gatherable - Open in Fishing Log".Tr());
                ...
                {
                    ImGui.TextUnformatted("Item ID: ".Tr() + _itemId);
        ...
        private void DrawSources()
        {
            ...
            if (ImGui.CollapsingHeader("Sources (".Tr() + Item.Sources.Count + ")", ImGuiTreeNodeFlags.DefaultOpen | ImGuiTreeNodeFlags.CollapsingHeader))
            {
            ...
            if (Item is { CanBeCrafted: true })
            {
                ...
                if (_craftTypes == null)
                {
                    ...
                    else
                    {
                        ...
                        {
                            craftTypes[recipe.RowId] = recipe.CraftType?.FormattedName ?? "Unknown Craft Type".Tr();
                        }
                    }
        ...
        private bool DrawSharedModels()
        {
            ...
            {
                hasInformation = true;
                if (ImGui.CollapsingHeader("Shared Models (".Tr() + SharedModels.Count + ")"))
                {
                    ImGuiStylePtr style = ImGui.GetStyle();
                    float windowVisibleX2 = ImGui.GetWindowPos().X + ImGui.GetWindowContentRegionMax().X;
        ...
        private bool DrawRecipes()
        {
            ...
            {
                hasInformation = true;
                if (ImGui.CollapsingHeader("Recipes - Item is a requirement (".Tr() + RecipesAsRequirement.Length + ")"))
                {
                    ImGuiStylePtr style = ImGui.GetStyle();
                    float windowVisibleX2 = ImGui.GetWindowPos().X + ImGui.GetWindowContentRegionMax().X;
        ...
        private bool DrawGatheringSources()
        {
            ...
            {
                hasInformation = true;
                if (ImGui.CollapsingHeader("Gathering (".Tr() + GatheringSources.Count + ")"))
                {
        ...
        private bool DrawRetainerTasks()
        {
            ...
            {
                hasInformation = true;
                if (ImGui.CollapsingHeader("Ventures (".Tr() + RetainerTasks.Count() + ")"))
                {
        ...
        private bool DrawVendors()
        {
            ...
            {
                hasInformation = true;
                if (ImGui.CollapsingHeader("Shops (".Tr() + Vendors.Count + ")"))
                {
                    ImGui.TextUnformatted("Shops: ".Tr());
        ...
        {
            if (ImGui.CollapsingHeader("Owned (".Tr() + OwnedItems.Count + ")"))
            {
        ...
        private void DrawOwnedItem(CriticalCommonLib.Models.InventoryItem obj)
        {
            ...
            if (obj.SortedCategory == InventoryCategory.GlamourChest && obj.GlamourId != 0)
            {
                ...
                if (ImGui.IsItemHovered())
                {
                    ...
                    {
                        ImGui.TextUnformatted("This item has been combined into a single outfit glamour item.".Tr());
                    }
                }
            }
            ImGui.TableNextColumn();
        ...
        void DrawSupplierRow((IShop shop, ENpcResidentRow? npc, ILocation? location) tuple)
        {
            ...
            if (tuple.npc != null && tuple.location != null)
            {
                ...
                ImGui.TableNextColumn();
                if (ImGui.Button("Teleport".Tr() + "##t" + tuple.shop.RowId + "_" + tuple.npc.RowId + "_" +
                ...
                if (ImGui.Button("Map Link".Tr() + "##ml" + tuple.shop.RowId + "_" + tuple.npc.RowId + "_" +
                                 tuple.location.Map.RowId))
                {
                    _chatUtilities.PrintFullMapLink(tuple.location, Item?.NameString ?? "");
                }
            ...
            {
                ImGui.TableNextColumn();
                ImGui.TextWrapped("Housing Vendor".Tr());
                ImGuiService.HelpMarker("This is a vendor that can be placed inside your house/apartment.".Tr());
                ImGui.TableNextColumn();
            }
            else
            {
        ...
        private void DrawMarketPricing()
        {
            if (Item is { CanBePlacedOnMarket: true })
            {
                        ...
                        ImGuiTreeNodeFlags.CollapsingHeader | ImGuiTreeNodeFlags.DefaultOpen))
                {
                    ...
                    ImGuiUtil.HoverTooltip("Refresh the current prices.".Tr());
        ...
        private void DrawMobDrops()
        {
            ...
            {
                if (ImGui.CollapsingHeader("Mob Drops (".Tr() + MobDrops.Length + ")", ImGuiTreeNodeFlags.CollapsingHeader))
                {
                     ...
                     for (var index = 0; index < mobDrops.Length; index++)
                     {
                         ...
                         if (bnpcName != null)
                         {
                             ...
                             {
                                 ImGui.TextUnformatted("No locations known.".Tr());
                             }
                         }
                     }
                }
        ...
        private void DrawUses()
        {
            ...
            if (ImGui.CollapsingHeader("Uses (".Tr() + Item.Uses.Count + ")", ImGuiTreeNodeFlags.DefaultOpen | ImGuiTreeNodeFlags.CollapsingHeader))
            {
